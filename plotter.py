...
def draw(   # general settings
    ...
    def linechart_plotter(x_axis):
        ...
        for i in range(len(dataset)):
            if i == 2 and len(dataset) == 3:
                plt.plot(x_axis[i], dataset[i], linewidth=linewidth, linestyle=linestyle_list[i], color=color_default[i+1], marker=marker_list[i], markersize=markersize)
            else:
    ...
    def scatter_plotter(x_axis):
        ...
        # plot scatter
        for i in range(len(dataset)):
            ax.scatter(x_axis, dataset[i], s=markersize, marker=marker_list[i])
    ...
    # styles setting
    if legends != []:
        plt.legend(legends,loc=legend_location, fontsize=legend_fontsize)
